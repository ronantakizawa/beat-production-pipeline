"""
Elevate — Compose (Progressive House)
Eb major | 128 BPM | 96 bars (~3:00)

Research: Path B — YouTube tutorials on Martin Garrix progressive house
...
Chord Progression (4-bar loop, Eb major — I-V-vi-IV):
  Bar 0: Eb  (Bb3, Eb4, G4)   — I  (2nd inv)
  Bar 1: Bb  (Bb3, D4, F4)    — V  (root)
  Bar 2: Cm  (C4, Eb4, G4)    — vi (root)
  Bar 3: Ab  (Ab3, C4, Eb4)   — IV (root)
...
9 Sound Layers:
  ...
  5. Lead melody — Eb major, chord-derived phrase
  6. Lead2      — same melody, softer layer (drops only)
  7. Pluck      — same melody, octave lower
  8. Pad        — sustained chords
...
BEAT_NAME  = 'Elevate'
OUTPUT_DIR = '/Users/ronantakizawa/Documents/Elevate_Beat'
...
# ============================================================================
# CHORD TABLES — Eb major I-V-vi-IV
# All voicings within ±7 semitones of C4 (MIDI 60) to avoid pitch-shift artifacts
# Reference: chordprogressions/GitHub Free Progressions/Eb/Eb Major/
# ============================================================================

CHORDS = [
    ['B-3', 'E-4', 'G4'],    # bar 0: Eb major (2nd inv — keeps Bb3 close to C4)
    ['B-3', 'D4',  'F4'],    # bar 1: Bb major (root pos)
    ['C4',  'E-4', 'G4'],    # bar 2: C minor (root pos)
    ['A-3', 'C4',  'E-4'],   # bar 3: Ab major (root pos)
]

BASS_ROOTS = ['E-1', 'B-1', 'C1', 'A-1']

# Pad voicings — one octave above CHORDS to keep out of bass range
PAD_CHORDS = [
    ['B-4', 'E-5', 'G5'],    # bar 0: Eb major (2nd inv)
    ['B-4', 'D5',  'F5'],    # bar 1: Bb major
    ['C5',  'E-5', 'G5'],    # bar 2: C minor
    ['A-4', 'C5',  'E-5'],   # bar 3: Ab major
]

# ============================================================================
# MELODY — derived from Eb Major chord tones (chordprogressions/ reference)
# ============================================================================
# Eb major scale: Eb F G Ab Bb C D
# Rule: chord tones on strong beats (1, 3), Eb major scale passing tones between.
# Each bar = list of (pitch_or_None, duration_in_beats)

LEAD_MELODY = [
    # Bar 0 (Eb I): start on 5th (Bb4), hold, step down — descending arch
    [('B-4', 2.0), ('G4', 0.5), ('E-4', 1.5)],
    # Bar 1 (Bb V): repeated Bb4 (stick-to-one-note), then leap to F5
    [('B-4', 0.5), ('B-4', 0.5), ('B-4', 1.0), ('F5', 2.0)],
    # Bar 2 (Cm vi): sustained C5, quick neighbor turn
    [('C5', 2.5), ('D5', 0.5), ('C5', 1.0)],
    # Bar 3 (Ab IV): short Ab4, long rest — breathing room
    [('A-4', 1.0), ('E-4', 1.0), (None, 2.0)],
]
...
# DRUMS
# ============================================================================
# GM mapping: 36=kick 39=clap 51=ride 42=closed-HH 46=open-HH 49=crash 38=snare
...
def create_drums():
    ...
    def buildup_hats(bar, bar_in_build, total_bars):
        ...
        o = bb(bar)
        # Progress 0.0 -> 1.0 across the buildup
        progress = (bar_in_build + 0.5) / total_bars
        base_vel = int(35 + progress * 50)  # 35 -> 85
        for i in range(16):
            # Accent every 4th hit for groove
            accent = 8 if (i % 4 == 0) else 0
            vel = min(110, base_vel + accent + random.randint(-3, 3))
            hit(o + i * 0.25, 42, vel)  # 42 = closed hi-hat
    ...
    # Buildup 1 (4 bars): kicks + 16th hi-hat crescendo
    ...
    # Buildup 2 (4 bars): bigger — kicks + 16th hi-hat crescendo (louder start)
...
def create_piano():
    ...
    def stretched_chords(s_bar, e_bar, vel=55):
        """Intro/outro: 2 bars per chord, only first 2 chords (Eb, Bb)."""
        for bar in range(s_bar, e_bar, 2):
            idx = ((bar - s_bar) // 2) % 2  # alternate Eb and Bb
...
def create_lead():
    ...
    def write_phrase(s_bar, e_bar, vel_range=(40, 55)):
        ...
        # shift octaves as [base, base, +1, +1] per workflow.md MANDATORY
        for bar in range(s_bar, e_bar):
            bar_idx = bar % 4
...
# ============================================================================
# LEAD2 — softer layer, same melody, drops only
# ============================================================================
...
def create_lead2():
    ...
    # Drops only — softer velocity than lead
    write_phrase(DROP1_S, DROP1_E, vel_range=(35, 48))
    write_phrase(DROP2_S, DROP2_E, vel_range=(38, 50))
...
if __name__ == '__main__':
    ...
    print(f'  Key: Eb major  |  BPM: {BPM}  |  96 bars')
