"""
Tainy Clone v2 — Render Script
Ab minor (1A) | 112 BPM | 105 bars

Kit: reggaetondrums (NO ME CONOCE) + lex_luger_drum_kit

MIDI track indices in Tainy_FULL.mid:
...
# ============================================================================
# CONFIG — from ref_analysis.json
# ============================================================================

BEAT_NAME = 'Tainy'
OUTPUT    = '/Users/ronantakizawa/Documents/Tainy_Beat'
...
# Section boundaries (matched to compose_tainy.py)
INTRO_S,    INTRO_E    =  0,  34
BRIDGE1_S,  BRIDGE1_E  = 34,  38
VERSE1_S,   VERSE1_E   = 38,  55
VERSE2_S,   VERSE2_E   = 55,  59
VERSE3_S,   VERSE3_E   = 59,  64
VERSE4_S,   VERSE4_E   = 64,  72
BRIDGE2_S,  BRIDGE2_E  = 72,  81
VERSE5_S,   VERSE5_E   = 81,  85
OUTRO_S,    OUTRO_E    = 85, 105

rng = np.random.RandomState(42)


# ============================================================================
# HELPERS — reuse from skeleton
# ============================================================================
...
# SUB BASS: sine osc, LPF 200Hz — from instruments.bass.type=sub_bass
# SUB BASS TIMBRE RULE: pure sine, fast gate, no harmonics above 200Hz
SUB_BASS_DSP = """
import("stdfaust.lib");
...
# PAD: 5x detuned saw, slow attack — from instruments.melody.pad.type=strings
# PAD TIMBRE RULE: detuned saw strings, slow attack 0.5s, release 3s,
# HPF at 150Hz. Never louder than drums.
PAD_DSP = """
import("stdfaust.lib");
freq = hslider("freq[unit:Hz]", 440, 0.001, 20000, 0.001);
gain = hslider("gain", 1, 0, 1, 0.01);
...
# LEAD: sine+FM, medium attack — from instruments.melody.main.type=lead_synth
...
REGDRUMS = os.path.join(INST, 'reggaetondrums')
LEX      = os.path.join(INST, 'lex_luger_drum_kit/lex_luger_drum_kit')
RAP2     = os.path.join(INST, 'rap2')

print('\nStep 1: Loading samples ...')
KICK   = load_sample(os.path.join(REGDRUMS, 'Bombos (Kicks)/NO ME CONOCE_KICK_3.wav'))
CLAP   = load_sample(os.path.join(REGDRUMS, 'Redoblantes (Snares)/NO ME CONOCE_SNARE_3.wav'))
HH_CL  = load_sample(os.path.join(REGDRUMS, 'Platos (Cymbals)/NO ME CONOCE_HH_3.WAV'))
HH_OP  = load_sample(os.path.join(RAP2, 'Open Hats/(OH) - Frida.wav'))
CRASH  = load_sample(os.path.join(LEX, 'LEX Crash.wav'))
SNARE  = load_sample(os.path.join(REGDRUMS, 'Redoblantes (Snares)/NO ME CONOCE_SNARE_5.wav'))
...
# ============================================================================
# STEP 4 — Sub Bass (FAUST synth, not 808)
# ============================================================================
...
def build_sub_bass(sc_gain):
    ...
    # EQ from mix_profile: bass LPF at 200Hz (sub_bass)
    ...
    # Force mono (stereo_width.sub = 0.021 from mix_profile)
...
def build_pad(sc_gain):
    ...
    # Intro filter sweep: LPF 200Hz→8000Hz over bars 0-26
    # From arrangement.intro_technique=filter_sweep_in
    intro_end_samp = int(bar_to_s(INTRO_E) * SR)
    if intro_end_samp > 0 and intro_end_samp < NSAMP:
        # Apply progressive LPF via pedalboard on intro segment
    ...
    # Pad effects — HPF 150Hz, reverb (1.02s decay → room_size ~0.55)
    pad_board = pb.Pedalboard([
        pb.HighpassFilter(cutoff_frequency_hz=150),
        pb.LowpassFilter(cutoff_frequency_hz=8000),
        pb.Reverb(room_size=0.55, damping=0.50, wet_level=0.30,
...
def build_lead(sc_gain):
    ...
    # Intro filter sweep on lead too
    intro_end_samp = int(bar_to_s(INTRO_E) * SR)
    if intro_end_samp > 0 and intro_end_samp < NSAMP:
        sweep_seg = buf[:intro_end_samp].copy()
        n_chunks = INTRO_E
        ...
        for ci in range(n_chunks):
            ...
            e = min((ci + 1) * chunk_size, intro_end_samp)
            progress = ci / n_chunks
            cutoff = 300 + progress * 7700  # 300 → 8000 Hz
            lpf = pb.Pedalboard([pb.LowpassFilter(cutoff_frequency_hz=cutoff)])
            chunk = sweep_seg[s:e]
            if len(chunk) > 0:
                sweep_seg[s:e] = apply_pb(chunk, lpf)
...
# ============================================================================
# TRANSITION FX — from arrangement.transitions
# ============================================================================

def build_transition_fx():
    """Transitions at section boundaries:
    riser→verse1@38, crash→verse2@55, fill→verse3@59, crash→bridge1@34,
    drop→bridge2@72, crash→verse4@64, riser→verse5@81, riser→outro@85."""
    ...
    # riser@24: snare roll + reverse tail into verse1
    snare_roll(VERSE1_S, bars_build=2.0)
    reverse_tail(VERSE1_S, length_beats=3)
    place(fx_L, fx_R, CRASH * 0.72,
          int(bar_to_s(VERSE1_S) * SR), 0.44, 0.56)

    # sweep_up@30: crash into verse2
    place(fx_L, fx_R, CRASH * 0.50,
          int(bar_to_s(VERSE2_S) * SR), 0.44, 0.56)

    # fill@40: snare density burst into verse3
    snare_roll(VERSE3_S, bars_build=1.0)
    place(fx_L, fx_R, CRASH * 0.45,
          int(bar_to_s(VERSE3_S) * SR), 0.44, 0.56)

    # sweep_up@46: crash into bridge1
    place(fx_L, fx_R, CRASH * 0.45,
          int(bar_to_s(BRIDGE1_S) * SR), 0.44, 0.56)

    # drop@51: crash (energy drops — drums+bass stop)
    place(fx_L, fx_R, CRASH * 0.35,
          int(bar_to_s(BRIDGE2_S) * SR), 0.44, 0.56)

    # sweep_up@57: crash (bass re-enters)
    place(fx_L, fx_R, CRASH * 0.50,
          int(bar_to_s(VERSE4_S) * SR), 0.44, 0.56)

    # riser→verse5@81: drums+bass re-enter after bridge2 drop
    snare_roll(VERSE5_S, bars_build=1.5)
    reverse_tail(VERSE5_S, length_beats=2)
    place(fx_L, fx_R, CRASH * 0.55,
          int(bar_to_s(VERSE5_S) * SR), 0.44, 0.56)

    # riser→outro@85: snare roll into outro fade
    snare_roll(OUTRO_S, bars_build=1.0)
    reverse_tail(OUTRO_S, length_beats=2)
...
def master_and_export(mix, target_lufs=-14.0):
    sections = [
        ...
        ('Verse3',   VERSE3_S,  VERSE3_E),
        ('Verse4',   VERSE4_S,  VERSE4_E),
        ('Bridge2',  BRIDGE2_S, BRIDGE2_E),
        ('Verse5',   VERSE5_S,  VERSE5_E),
        ('Outro',    OUTRO_S,   OUTRO_E),
    ]

    # Master chain (NO limiter yet)
    print('Master chain ...')
    ...
    # Fade out over outro section (bars 68-79)
    # From arrangement.outro_technique=fade_out
    ...
    # LUFS normalization — limiter first, then measure and normalize
    ...
    # Step 1: true-peak limiter (controls peaks, increases avg loudness)
    ...
    # Step 3: apply gain to hit target (gain will be negative = quieter)
